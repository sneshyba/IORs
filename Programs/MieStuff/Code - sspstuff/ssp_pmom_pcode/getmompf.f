...
C Variables for input/output
      INTEGER     MAXTABLEG, NPHASE, VERBOSE, DONOR
      REAL*8      PHASE(*)
      REAL*8      ANGLE(*), SIZEPARAM
      REAL*8      THETADEG

      REAL*8,     INTENT(OUT), DIMENSION(NPHASE) :: NEWPHASE  ! Added by SPN 2015 for f2py
      REAL*8,     INTENT(OUT), DIMENSION(0:MAXLEG) :: LEGEN ! Added by SPN 2015 for f2py
      INTEGER,    INTENT(OUT) :: NLEG   ! Added by SPN 2015 for f2py
...
C     What happens if the IF statement below fails for all L?
C     Is NLEG assigned to 0?  Or whatever is in memory?
C     I.e. should NLEG be set to 0 before the loop? PMR 2015/04/27
C     NLEG = 0
      DO L = 0, NLEGEN
